...
export default function TabButton<N extends string>({ name, icon }: TabDef<N>) {
  ...
  return (
    <button
      className={`px-1 pt-1.5 pb-2 ${isActive ? 'text-accent font-bold' : 'text-gray-600 font-semibold'}`}
      aria-current={isActive ? 'page' : undefined}
      onClick={() => changeTab(name)}
    >
      <div
        className={`flex min-w-12 justify-center rounded-full py-1 ${isActive ? 'bg-accent/12' : ''}`}
      >
        {icon}
      </div>
