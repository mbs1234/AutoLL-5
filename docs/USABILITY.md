...
|---|---|---|---|
| 1 | Words | Typos and wrong limits; sentences that point somewhere become buttons that go there; Disney's eligibility codes in plain words; dates in words; NextLL's time bounds say what they do; the iPhone alert line; a Plan Check button on its own line; menu rows tappable edge to edge | merged (#10) |
| 2 | Recovery | Errors that stay, with their time, until dismissed; lists that say when they have not loaded instead of that they are empty; the booking screen telling a dropped connection from a sold-out ride; confirmation for Cancel, Log Out and turning Autopilot off; a stopped run that says how to restart, in red on every tab; a note on Today when a sign-in expiry stopped Autopilot; Plans buttons wherever an outcome is in doubt | merged (#11) |
| 3 | Today | Tappable passes and plan rows; how long a held pass has left (roadmap item 3, the screen half); a park-morning check in the status card (item 13); the plan's own park named when the header shows another (item 14); no drop time for a date the engine never bursts for, and its two drop times named apart (item 8's honesty half); a line saying why nothing has booked; the readiness list on a first run, with a backup row; a Guided Access reminder in Pocket mode on iPhone | merged (#12) |
| 4 | Reach | 44 px tap areas on every small button (the chip keeps its size) and 44 px time and rank boxes; the park, day and party chip as a control on Today, LL and Times; Sort out of the LL header, which squeezed the title to one letter; the Autopilot light as a dot and the armed count, amber at none; planned days marked in the calendar; the green bar in the legend, a readable tier badge, a turning Configure arrow, a red Replace on Restore, and Down in words on the Times tab | merged (#13) |
| 5 | Setup | One "What should Autopilot do?" choice of five in place of three overlapping chips, named as the summaries name them; a just-added ride says it only alerts until one is chosen; under an armed Swap in, which pass it would give up now, from the engine's own `chooseSwapVictim`; no star in Configure (a + row adds, a word removes); an undo for every removal in the moment; a setting from Plan Check opens at the safeguards | merged (#14) |
| 6 | NextLL | A running search asks before a tab or the footer row leaves it (a leave guard in `withTabs`); who holds each pass in the modify list; "Move this one" when several hold a ride; a "Modifying …" strip on every tab while a hand-started modify lasts, and the booking screen saying what it replaces; dry run said plainly on NextLL; a met goal says it keeps looking until Done; no duplicate back button | merged (#15) |
| 7 | Names | "Book again at" for the next booking window, which had three names; "Watching" for Autopilot's list on Today, so "Plans" is only Disney's; "Find a better time" for both ways into Time Search; "Modify" split into Swap ride, Edit party and Change time, with commits that say what they do ("Move to 1:10 PM", "Replace Haunted Mansion"); a NextLL tab icon of its own | merged (#16) |
